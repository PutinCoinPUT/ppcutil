...
import (
	...
	"github.com/PutinCoinPUT/ppcutil/base58"
...
var masterKey = []byte("Peercoin seed")
...
func (k *ExtendedKey) Derive(i uint32) (*ExtendedKey, error) {
	...
	parentFP := ppcutil.Hash160(k.pubKeyBytes())[:4]
...
func (k *ExtendedKey) Address(net *chaincfg.Params) (*ppcutil.AddressPubKeyHash, error) {
	pkHash := ppcutil.Hash160(k.pubKeyBytes())
	return ppcutil.NewAddressPubKeyHash(pkHash, net)
}
